...
CDlgEmaPara::CDlgEmaPara(RpsGroup rg, HWND hWnd) : SHostWnd(_T("LAYOUT:dlg_emaPara"))
{
	m_rgGroup = rg;
	m_hParWnd = hWnd;
}
...
void CDlgEmaPara::OnClose()
{
	CSimpleWnd::DestroyWindow();
}

void CDlgEmaPara::OnClickButtonOk()
{
	...
	::SendMessageW(m_hParWnd, WM_FENSHI_MSG, 
		(WPARAM)nPara, FSMSG_EMA);
	OnClose();
}

void CDlgEmaPara::OnClickButtonCancel()
{
	OnClose();

}

void CDlgEmaPara::OnInit(EventArgs * e)
{
...
void CDlgEmaPara::OnClickButtonDefault()
{
	SEdit *para1 = FindChildByName2<SEdit>(L"edit_EMAPara1");
...
void CDlgEmaPara::OnClickButtonSetDefault()
{
	int tmpPara[2];
	SEdit *para1 = FindChildByName2<SEdit>(L"edit_EMAPara1");
	strPara1 = para1->GetWindowTextW();
...
void CDlgEmaPara::SetEditText(const int * pPara)
{
	SEdit *para1 = FindChildByName2<SEdit>(L"edit_EMAPara1");
...
void CDlgEmaPara::OnFinalMessage(HWND hWnd)
{
