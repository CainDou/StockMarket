...
void CDlgUpdateLog::UpdateLog()
{
	...
	strLog.Format(
		L"最新更新\n"
		L"时间:2026-07-10\n更新内容:\n"
		L"1、在砖型图中添加了吊灯止损和生成时间指标\n"
		L"2、在模拟交易中添加了对ETF的支持"
		L"--------------------------------------------------------------\n"
		L"历史更新\n"
		L"时间:2026-06-10\n更新内容:\n"
		L"1、在股票中添加了秒级K线及砖型图的显示\n"
		L"时间:2025-11-20\n更新内容:\n"
		L"1、添加了多周期分析功能，能够在同时在四个不同周期中实现K线显示，并保持时间轴同步\n"
		L"时间:2025-08-19\n更新内容:\n"
		L"1、添加了NetGrid交易策略选股\n"
		L"2、在股票行情客户端和期货行情客户端中添加了网格交易策略图表，并优化了策略计算架构\n"
		L"3、修复了股票行情客户端在某些情况下闪退的问题\n"
		L"4、修复了期货行情客户端部分新增品种没有沉淀资金显示的问题\n"
		L"时间:2025-07-28\n更新内容:\n"
		L"1、添加了交易系统选股功能，添加了SideWinder交易策略选股\n"
		L"2、将超级趋势布林线策略在原来的选股基础上添加到了交易系统选股功能中\n"
		L"3、添加了自选股功能，能够将策略选出的股票添加到自选股中，并观察自添加以来的涨跌变化\n"
		L"4、修复了已知bug,优化了主图的行情列表的更新功能，减少更新带来的列表位置变化\n"
		L"时间:2025-05-19\n更新内容:\n"
		L"1、在分时图主图中添加了每分钟的委托均价、委买均价、委卖均价和撤买撤卖均价的价格线显示\n"
		L"2、在分时图添加了每分钟的委托量、委托单、撤单量和撤单单数的指标显示\n"
		L"3、添加了资金流量价分布图表，能够按照委托、撤单和成交任意组合显示，在每个价位的数量情况，能够按照最新分钟和截至目前两种时间显示\n"
		L"4、可以通过主界面“资金流”按钮或者按快捷键F3进入资金流量价分布图表\n"
		L"时间:2024-11-06\n更新内容:\n"
		L"1、添加了对超级趋势布林线策略的选股功能，添加了多开、空开、多平、空平四个方向的选股指标\n"
		L"时间:2024-05-06\n更新内容:\n"
		L"1、添加了对多个服务器连接的支持，在主服务器无法连接时，自动连接副服务器\n"
		L"2、修复了打分数据图表在鼠标离开绘图位置时，显示的数据可能不对的问题\n"
		L"时间:2024-04-29\n更新内容:\n"
		L"1、全面修改了服务器的架构，提升服务器处理性能，提高服务器并行运算多核调用能力\n"
		L"2、对新版服务器进行了适配，修改了部分函数的处理\n"
		L"3、修复切换部分指标时，程序闪退的问题\n"
		L"4、修复了使用订单流历史数据指标时，数据显示错误的问题\n"
		L"时间:2024-03-18\n更新内容:\n"
		L"1、添加了大单的买卖成交差在日线上的指标图形\n"
		L"时间:2024-02-19\n更新内容:\n"
		L"1、修复了在使用参数基准价格为无时，显示的图形可能无法覆盖整个价格区间的问题\n"
		L"时间:2024-02-08\n更新内容:\n"
		L"1、在服务器端完成了长周期的价量分布数据的计算与处理，并修复了内存泄露的问题\n"
		L"2、添加了长周期价量分布数据的显示，并能按照参数进行修改\n"
		L"3、添加了不同周期的图形之间的对比功能，能够更好的反应数据的变化\n"
		L"时间:2023-12-20\n更新内容:\n"
		L"1、在服务器端添加了股票实时价量数据分布的计算\n"
		L"2、优化了服务器端的内存表现与部分功能的处理速度\n"
		L"3、在rps客户端中添加了包含主被动与大中小单的量价数据图的显示，可以使用F2键进行调用\n"
		L"4、优化了rps客户端的主界面与其他独立界面的信息处理模块\n"
		L"时间:2023-11-08\n更新内容:\n"
		L"1、在服务器端完成了选股公式数据的实时计算\n"
		L"2、完成了选股公式历史的中间数据的计算\n"
		L"3、在rps客户端的选股器中添加了选股公式数据的调用，并对条件的添加作了相应的限制\n"
		L"4、修复了部分界面的现实问题\n"
		L"时间:2023-10-31\n更新内容:\n"
		L"1、修复了在切换界面保留代码后，直接报单无法识别相应价格，导致报单出现超出限制的问题\n"
		L"时间:2023-10-27\n更新内容:\n"
		L"1、修复了页面切换清空数据选项无法修改只能设置为是的问题\n"
		L"2、修复了在页面切换清空数据设置为是的情况下，点击信息栏时，会清空已输入信息的问题\n"
		L"时间:2023-10-11\n更新内容:\n"
		L"1、优化了委托确认，添加了通过Enter和Esc键进行控制的功能\n"
		L"时间:2023-10-10\n更新内容:\n"
		L"1、修复了持仓错误的问题\n"
		L"2、添加了推送的对话框自动关闭和委托确认功能\n"
		L"时间:2023-10-09\n更新内容:\n"
		L"1、添加了交易的设置界面，优化了交易功能，能够按需进行交易设置\n"
		L"2、修复了成交汇总数据显示与实际成交不符的问题\n"
		L"3、修复了服务器端进行带价成交时数量错误的问题\n"
		L"4、优化了持仓显示，去掉了已清仓股票的显示\n"
		L"时间:2023-09-25\n更新内容:\n"
