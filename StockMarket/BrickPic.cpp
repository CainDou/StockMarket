...
SOUI::BrickPic::BrickPic() :
	...
	m_pHisRenkoVec(nullptr), m_bHalfPrice(false)
{
	m_bFocusable = 1;
...
void SOUI::BrickPic::AddData(const RenkoData& data, int64_t time)
{
	...
	else
	{
		...
		{
			UpdatePrice(data.fStart, time, fUpper, fLower, fUpperStart, fLowerStart, data.fStart != Close.back());
			m_fPreStart = data.fStart;
		}
		UpdatePrice(data.fEnd, time, fUpper, fLower, fUpperStart, fLowerStart, false);
...
void SOUI::BrickPic::AddData(double fPrice, int64_t time, bool bJump)
{
	...
	else
	{
		...
		double fUpper = fUpperStart + GetBrickStep(fUpperStart);
		double fLower = fLowerStart - GetBrickStep(fLowerStart);
		UpdatePrice(fPrice, time, fUpper, fLower, fUpperStart, fLowerStart, bJump);
	}
}
...
void SOUI::BrickPic::AddBrickDataToEmpty(const RenkoData& data, int64_t time)
{
	...
	for (int i = 0; i < fPriceVec.size(); ++i)
		UpdatePrice(fPriceVec[i], time, fUpper, fLower, fUpperStart, fLowerStart, false);
...
SStringW SOUI::BrickPic::GetYPrice(int nY)
{
	...
	double fDiff = m_fMinY + (double)(m_rcMain.bottom - nY - RC_MIN)
		/ nHeight * fPriceDiff;
	if (fDiff < 1000)
...
void SOUI::BrickPic::DrawUpperMarket(IRenderTarget* pRT, int nX)
{
	...
	strMarket.Format(L"%s 砖型图(%g%s) 日期:%04d-%02d-%02d 时间:%02d:%02d:%02d 开:%.02f 高:%.02f 低:%.02f 收:%.02f",
		...
		Open[nX], High[nX], Low[nX], Close[nX]);
